...
Ho = The distribution of private room listings and Entire House listings in June 2019 are spatially independent.

H1= The distribution of private room listings and Entire House listings in June 2019 are NOT spatially independent.
...
```{r}
Combined_PR_EH_Lcross_2019.csr <- envelope(Combined_PR_EH_ppp, Lcross, i="PR_2019", j="EH_2019", correction='border', nsim=99)

```

```{r}
plot(Combined_PR_EH_Lcross_2019.csr, xlab="distance(m)", xlim=c(0,500))
```
...
The above steps can be repeated for the other room types as well to determine if they are spatially independent. For the putpose of this assignment, it will not be showcased due to time constraint.