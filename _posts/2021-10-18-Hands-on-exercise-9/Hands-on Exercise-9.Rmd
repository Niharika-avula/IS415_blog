...
description: |
 "In this hands-on exercise, I will learn to how to calibrate geographically weighted regression models by using GWmodel package of R."
author:
  - name: Niharika Avula 
...
* building hedonic pricing models by using GWR methods. 
* dependent variable is the resale prices of condominium in 2015 and the independent variables are divided into either structural and locational
...
Verifying the projection of the newly transformed mpsz_svy21:

```{r}
st_crs(mpsz_svy21)
```
Notice that the EPSG: is indicated as 3414 now, the extent of mpsz_svy21 by using st_bbox() of sf package
...
### 3.2.1 Converting aspatial data frame into a sf object

Currently, the condo_resale data frame is aspatial. We will convert it to a sf object with the code chunk below which converts condo_resale data frame into a simple feature data frame by using st_as_sf() of sf packages. Notice that st_transform() of sf package is used to convert the coordinates from wgs84 (i.e. crs:4326) to svy21 (i.e. crs=3414).
...
Now, plotting the LOG_SELLING_PRICE using the code chunk below.
...
### 4.1.1 Multiple Histogram Plots distribution of variables
...
The code chunk below is used to create 12 histograms. Then, *ggarrange()* is used to organise these histogram into a 3 columns by 4 rows small multiple plot.
...
In this section, hedonic pricing models will be built for condominium resale units using lm() of R base.
...
### 5.2.2 Building a hedonic pricing model using multiple linear regression method
...
```
### 5.2.3 Checking for multicolinearity
...
### 5.2.4 Testing for Non-Linearity
...
### 5.2.5 Testing for Normality Assumption
...
### 5.2.6 Testing for Spatial Autocorrelation
...
The code churn below will turn on the interactive mode of tmap and create an interactive point symbol map..
...
In this section, hedonic pricing will be modelled using both the fixed and adaptive bandwidth schemes
...
## 6.2 Build Adaptive Bandwidth GWR Model
